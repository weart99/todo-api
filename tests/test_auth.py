...
def test_register_user_existing_username(client, test_user_data):
    ...
    response = client.post("/auth/register", json=test_user_data)  # Try to register again
...
def test_register_user_existing_email(client, test_user_data):
    ...
    response = client.post("/auth/register", json=new_user_data)  # Try to register again
    assert response.status_code == 400
